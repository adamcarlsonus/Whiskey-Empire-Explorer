...
test("popup injects content, sends START_SCAN, and surfaces the returned failure", async () => {
  ...
  Reflect.set(globalThis, "chrome", {
    tabs: {
      ...
      sendMessage: async (_tabId: number, message: { type: string }) => {
        ...
        return { ok: true, snapshot: { status: "unsupported", pagesDiscovered: 0, pagesProcessed: 0, entriesCollected: 0, skippedCandidates: 0, warning: null, error: { code: "UNSUPPORTED_STRUCTURE", message: "This live menu structure is unsupported." } } };
      }
    },
  ...
  try {
    ...
    assert.equal(document.querySelector("#status")?.textContent, "This live menu structure is unsupported.");
  } finally {
