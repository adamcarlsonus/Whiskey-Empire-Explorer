...
export function createPanel(before: Element, actions: PanelActions): PanelView {
  ...
  shadow.innerHTML = `<style>${panelStyles}</style>
    <section class="panel" aria-labelledby="wew-heading">
      ...
      <div class="results-wrap"><table hidden id="wew-table"><thead><tr><th id="wew-name-head" scope="col">Name</th><th scope="col">Distillery</th><th scope="col">Proof</th><th id="wew-price-head" scope="col">Price</th><th scope="col">Notes</th></tr></thead><tbody id="wew-body"></tbody></table></div>
    </section>`;
